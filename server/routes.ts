...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Partnership Blog API Routes
  app.get("/api/partnership/blogs", async (req, res) => {
    try {
      const blogs = await storage.getAllPartnershipBlogs();
      // Transform blogs to include author and category data
      const blogsWithRelations = await Promise.all(blogs.map(async (blog) => {
        const author = blog.authorId ? await storage.getAllBlogAuthors().then(authors => authors.find(a => a.id === blog.authorId)) : null;
        const category = blog.categoryId ? await storage.getAllBlogCategories().then(categories => categories.find(c => c.id === blog.categoryId)) : null;
        ...
        return {
          ...
          tags: [] // Will implement tag relations later
        };
      }));
      
      res.json(blogsWithRelations);
    } catch (error) {
      res.status(500).json({ error: "Failed to fetch partnership blogs" });
    }
  });

  app.get("/api/partnership/blogs/featured", async (req, res) => {
    try {
      const blogs = await storage.getFeaturedPartnershipBlogs();
      res.json(blogs);
    } catch (error) {
      res.status(500).json({ error: "Failed to fetch featured partnership blogs" });
    }
  });

  app.get("/api/partnership/blogs/:slug", async (req, res) => {
    try {
      const blog = await storage.getPartnershipBlogBySlug(req.params.slug);
      if (!blog) {
        return res.status(404).json({ error: "Blog post not found" });
      }
      res.json(blog);
  ...
  app.get("/api/partnership/categories", async (req, res) => {
    try {
  ...
  app.get("/api/partnership/tags", async (req, res) => {
    try {
  ...
  app.get("/api/partnership/authors", async (req, res) => {
    try {
      const authors = await storage.getAllBlogAuthors();
      res.json(authors);
    } catch (error) {
  ...
  // Admin blog management routes (will expand later)
  app.post("/api/admin/partnership/blogs", async (req, res) => {
    try {
      const validatedData = insertPartnershipBlogSchema.parse(req.body);
      const blog = await storage.createPartnershipBlog(validatedData);
      res.status(201).json(blog);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res.status(400).json({ error: "Invalid blog data", details: error.errors });
      }
      res.status(500).json({ error: "Failed to create blog post" });
    }
  });
